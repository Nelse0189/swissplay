...
import { getTeamByManagerDiscordId, getManagerTeams } from '../lib/firebase-helpers.js';
...
export async function handleInviteRingerSelect(interaction) {
  ...
  const { getManagerTeams } = await import('../lib/firebase-helpers.js');
  const managerTeams = await getManagerTeams(interaction.user.id);
...
export async function handleInviteRingerTeamSelect(interaction, customId) {
  ...
  const db = admin.firestore();
  const { getManagerTeams, getUserByDiscordId } = await import('../lib/firebase-helpers.js');
